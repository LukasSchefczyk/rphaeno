...
```
das Tibble in die Datenbank geschrieben werden. (Anmerkung die, chunk-option eval=FALSE, wird also nicht ausgeführt aus Perfomancegründen).
...
```
```{r}
tbl(con,"Daten_Jahresmelder") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Megaframe_Jahresmelder") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Stationen") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Pflanze") %>% print(5,width=Inf)
``` 
```{r}
tbl(con,"Phasendefinition") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Mais_Spezifizierung") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Weinrebe_Spezifizierung") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Ruebe_Spezifizierung") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Phaenologie_Besonderheiten_Zeitreihen") %>% print(5,width=Inf)
```
```{r}
tbl(con,"Notizen") %>% print(5,width=Inf)
```
